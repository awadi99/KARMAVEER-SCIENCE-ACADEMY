...
export default function VisionMission() {
    // Subtle Mint and Slate glows to add depth to the dark background
    const bgGlows = useMemo(() => (
        <>
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-[radial-gradient(circle,_var(--tw-gradient-stops))] from-emerald-500/10 dark:from-emerald-400/5 to-transparent pointer-events-none transform-gpu" />
            <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-[radial-gradient(circle,_var(--tw-gradient-stops))] from-slate-500/10 dark:from-slate-800/10 to-transparent pointer-events-none transform-gpu" />
        </>
    ), []);

    return (
        /* Dark Color: Deep Obsidian (#0F172A) | Light Mode: Very Light Sage (#F8FAFA) */
        <section className="py-32 px-6 bg-[#F8FAFA] dark:bg-[#0F172A] overflow-hidden relative contain-paint transition-colors duration-500">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="text-left mb-20 max-w-2xl transform-gpu">
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        /* Uses Light Green (Emerald) for the badge */
                        className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white dark:bg-emerald-950/30 border border-slate-200 dark:border-emerald-500/20 shadow-sm mb-6"
                    >
                    ...
                    <h2 className="text-4xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-[1.1] mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12 items-stretch">
                    ...
                    {/* MISSION CARD (Light Green Accent on White/Dark) */}
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.5 }}
                        className="md:col-span-3 p-10 md:p-14 rounded-[2rem] bg-white dark:bg-[#1E293B] border border-slate-200/60 dark:border-white/5 shadow-xl dark:shadow-none group relative overflow-hidden transform-gpu"
                    >
                        <div className="absolute -top-16 -right-16 w-48 h-48 bg-emerald-50 dark:bg-emerald-900/10 rounded-full transition-transform duration-500 group-hover:scale-110 transform-gpu" />
                    ...
                    {/* VISION CARD (Solid Deep Dark with Light Green Text) */}
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.5, delay: 0.1 }}
                        /* Vision Card: Deep Dark Background (#020617) */
                        className="md:col-span-2 p-10 rounded-[2rem] bg-slate-900 dark:bg-[#020617] border border-slate-800 dark:border-emerald-500/30 shadow-2xl relative overflow-hidden group transform-gpu"
                    >
                        <div className="absolute top-0 right-0 w-40 h-40 bg-emerald-400/5 rounded-full -mr-20 -mt-20 group-hover:scale-110 transition-transform duration-500 transform-gpu" />
